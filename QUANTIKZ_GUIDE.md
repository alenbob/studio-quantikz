...
- `\textsc{UNIFORM}_M` applied to `\ket{0}` is interpreted as the normalized symbolic sum `\frac{1}{\sqrt{M}}\sum_{m=0}^{M-1}\ket{m}`. When the wire has a named subscript (e.g., `\ket{0}_\ell`), that subscript is used as the summation index instead of the first alphabetic letter of the count symbol, giving `\frac{1}{\sqrt{M}}\sum_{\ell=0}^{M-1}\ket{\ell}`.
...
- `In`, `In_a`, `\text{In}`, `\mathrm{In}`, or `\operatorname{In}` (bare or with any subscript) is treated as a transparent read gate — it acts as the identity on its row and does not transform the qubit state.
- `data:add_a` (also `\text{data:add}_a`, `\mathrm{data:add}_a`, `\operatorname{data:add}_a`) connected to another row via `\wire[d][n]{q}` infers the symbolic value `a` from the connected row's current state and writes `\ket{a}` to the target if it is in `\ket{0}` (since `0 \oplus a = a`), or `\ket{k \oplus a}` if the target holds `\ket{k}`. When no connector is present, the gate uses the subscript from the label directly as `a`. A bare `data:add` with no connector and no subscript falls back to opaque operator application.
